...
# Verify
kubectl describe nodepool ondemand-nodepool
Note: Existing nodes are not affected. New nodes use updated config. To force immediate rollout of new configuration:

Cordon and drain existing nodes, OR
Delete the NodePool and recreate it (disruptive)
Step-11: Key Takeaways
Layered Architecture - Separate Terraform projects for VPC, EKS, and Karpenter enable independent lifecycle management

Subnet Tags Matter - Use owned (not shared) for subnets where Karpenter provisions nodes

Spot Interruption Handling - SQS + EventBridge integration enables graceful spot node replacement

Pod Identity - Modern alternative to IRSA for secure AWS API access

Cost Optimization - Karpenter's diverse instance selection and consolidation significantly reduces compute costs

Rapid Scaling - Nodes provision in 30-60 seconds vs 3-5 minutes with Cluster Autoscaler

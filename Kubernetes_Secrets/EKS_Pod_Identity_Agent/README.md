...
Pod Requests Credentials Inside the pod, the AWS SDK/CLI uses the default credential provider chain.
...
Pod Accesses AWS Resources The AWS SDK/CLI inside the pod now has valid, short-lived credentials and can call AWS services (e.g., list S3 buckets).
...
Amazon EKS Pod Identity Blog Post

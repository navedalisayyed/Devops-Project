ntegrate AWS Secrets Manager with Catalog Microservice (EKS Pod Identity)
In the previous step (09-03), we installed the Secrets Store CSI Driver and AWS Secrets Provider (ASCP). Now, let’s use that setup to mount secrets directly from AWS Secrets Manager into our Catalog microservice.

In this section, we’ll securely connect AWS Secrets Manager with our Kubernetes Pods to provide MySQL credentials without ever storing them inside Kubernetes Secrets. This is the production-grade, zero-trust setup — credentials live only in AWS, and are fetched dynamically inside the container via the AWS Secrets and Configuration Provider (ASCP).
...
AWS Secrets Manager for AWS EKS Cluster
